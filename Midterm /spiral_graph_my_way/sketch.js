...
var fund = 0.065; // the speed of the central sine
...
function preload() {
  soundFile = loadSound('./data/170858__setuniman__happy-again-0x20mbc.mp3');
}
...
function draw()
{
  ...
  // make 2 seconds of squares
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    square = Math.floor(sines[i]);
    var erad = 0; // radius for small "point" within circle... this is the 'pen' when tracing
    // setup for tracing
    if (trace) {
      stroke(random(0,150), random(0,150), 255*(float(i)/sines.length), alpha); // blue
      fill(random(255), random(0,150), 255, alpha/2); // also, um, blue
      erad = 15.0*(1.0-float(i)/sines.length); // pen width will be related to which sine
    }
    if (square % 3 === 0) {
      var radius = rad/(i+2); // radius for circle itself
      rotate(sines[i]); // rotate circle
      if (!trace) ellipse(0, 0, radius*2, radius*2); // if we're simulating, draw the sine
      push(); // go up one level
      translate(0, radius); // move to sine edge
      if (!trace) ellipse(0, 0, sines[i]*5, 5); // draw a little circle
    ...
    else
    {
      ...
      push(); // go up one level
      translate(0, radius); // move to sine edge
      if (!trace) rect(0, 0, sines[i]*5, 5); // draw a little circle
